...
//    save：  不会去重，保存所有数据
     ...
     * @param bean
     */
    public static void insertData(ServersDBBean bean) {
     ...
     * @param bean
     */
    public static void insertOrReplaceData(ServersDBBean bean) {
    ...
    public static void deleteData(ServersDBBean bean) {
    ...
    public static void updateData(ServersDBBean bean) {
    ...
    public static List queryRaw(ServersDBBean bean, Long id) {

        List<ServersDBBean> beanLis = (List<ServersDBBean>)
                MyApplication.getInstance().getDaoSession().queryRaw(ServersDBBean.class, " where id = ?", id + "");
        return beanLis;
    }
     ...
     * @return
     */
    public static ServersDBBean queryListByMessageToGetPassword(String name) {
        boolean isExist = queryListIsExist(name);
        if (isExist) {
            List<ServersDBBean> ServersDBBeanList = queryListByMessage(name);
            Log.e("path=====:=====", ServersDBBeanList.size() + ""); //   /storage/emulated/0/1604026573438.mp4
            for (int i = 0; i < ServersDBBeanList.size(); i++) {

                return ServersDBBeanList.get(0);

            }
        } else {
            return new ServersDBBean();
        }
        return new ServersDBBean();
     ...
     * @return
     */
    public static List<ServersDBBean> queryListByMessage(String name) {
        DaoSession daoSession = MyApplication.getInstance().getDaoSession();
        QueryBuilder<ServersDBBean> qb = daoSession.queryBuilder(ServersDBBean.class);
        List<ServersDBBean> students = daoSession.queryRaw(ServersDBBean.class, " where username = ?", name);
        return students;
    }
